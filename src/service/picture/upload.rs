...
pub async fn post_picture(State(state): State<Arc<ServerState>>, mut multipart: Multipart) -> Result<String, (StatusCode, String)> {
        ...
        let data = match field.bytes().await {
            ...
            Err(err) => {
                return if &err.body_text() == "failed to read stream" {
                    ...
                    Err((StatusCode::INTERNAL_SERVER_ERROR, err.body_text().to_string()))
                };
            }
        };
        ...
        match name.as_str() {
            "file" => {
                if !file_type.clone().unwrap().starts_with("image/") {
                    return Err((StatusCode::BAD_REQUEST, "Invalid file type: ".to_string() + &file_type.clone().unwrap()));
                }
                file = Some(data);
    ...
    if UserImage::find().filter(crate::model::user_image::Column::FileName.eq(&file_name)).one(&state.db).await.unwrap().is_some() {
        return Err((StatusCode::CONFLICT, "File already exists.".to_string()));
    }

    let id = save_file(&file).await.unwrap();
    if Image::find_by_id(&id).one(&state.db).await.unwrap().is_none() {
        ...
        let image = Image::find_by_id(&id).one(&state.db).await.unwrap().unwrap();
